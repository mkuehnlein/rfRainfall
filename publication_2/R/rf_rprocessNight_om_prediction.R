...
path.in <- "/home/mkuehnlein/casestudies/rf_input/vp03/night/om/"
path.out <- "/home/mkuehnlein/randomForest/rf_rprocess/results/vp03/process3/night/om/"
...
for (i in 1:30){
  ...
  # READ DATA
  filename <- paste(paste(paste(path.in,"rfInput_night_om_vp03_train_",sep=""),v,sep=""),".dat",sep="")
...
file.copy("rf_rprocessNight_om_prediction.Rout", path.out.akt, overwrite = TRUE, copy.mode = TRUE)

}

## copy Rout #####################
file.copy("rf_rprocessNight_om_prediction.Rout", path.out, overwrite = TRUE, copy.mode = TRUE)
file.remove("rf_rprocessNight_om_prediction.Rout")
