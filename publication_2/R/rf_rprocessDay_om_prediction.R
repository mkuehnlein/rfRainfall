...
path.out <- "/home/mkuehnlein/randomForest/rf_rprocess/results/vp03/process3/day/om/"
...
for (i in 13:30){
  ...
  ## prediction ##
  # READ DATA
  filename <- paste(paste(paste(path.in,"rfInput_day_om_vp03_test_",sep=""),v,sep=""),".dat",sep="")
...
file.copy("rf_rprocessDay_om_prediction.Rout", path.out.akt, overwrite = TRUE, copy.mode = TRUE)

}

## copy Rout #####################
file.copy("rf_rprocessDay_om_prediction.Rout", path.out, overwrite = TRUE, copy.mode = TRUE)
file.remove("rf_rprocessDay_om_prediction.Rout")
